...
public class ButtonBuilder {
	...
	public Button build() {
		Button btn = new Button(userAction.toString());
	...
	public Button buildExitBtn(int translateX, int translateY) {
		Button btn = new Button(UserAction.EXIT_APPLICATION.toString());
		btn.setTranslateX(translateX);
		btn.setTranslateY(translateY);
		btn.setOnAction(action -> UserConfirmation.confirm(SystemNotificationType.CONFIRM_EXIT_APPLICATION));
