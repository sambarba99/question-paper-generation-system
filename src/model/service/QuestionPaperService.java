...
public class QuestionPaperService {
	...
	private QuestionPaperService(QuestionPaperDAO questionPaperDao) {
		assertNotNull(questionPaperDao);
	...
	/**
	 * Add a question paper to the question papers XML file.
	 * 
	...
	/**
	 * Delete question papers by their unique IDs.
	 * 
	 * @param ids - the IDs of the papers to delete
	 */
	public void deleteQuestionPapersByIds(List<Integer> ids) {
		questionPaperDao.deleteQuestionPapersByIds(ids);
	}

	/**
	 * Retrieve all question papers from question papers XML file.
	 */
	...
	public List<QuestionPaperDTO> getQuestionPaperDTOsWithSubjectFilter(List<Integer> subjectIds) {
		 ...
		 * subject. This is why 'subjectIds.isEmpty()' is in a logical disjunction (||).
		 */
		return getAllQuestionPapers().stream()
	...
	public String getQuestionPaperDisplayStr(QuestionPaper questionPaper) {
		...
		resultBld.append("\nApprox. Bloom skill level: " + questionPaper.getSkillLevel().toString());
		...
		for (int i = 0; i < questionIds.size(); i++) {
			...
			if (questionOpt.isPresent()) {
				...
				for (int j = 0; j < question.getAnswers().size(); j++) {
					resultBld.append("\n(" + ((char) (Constants.ASCII_UPPER_A + j)) + ") "
